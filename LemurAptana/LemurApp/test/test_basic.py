...
class OrderTest(TestCase):
  ...
  def test_order_cleanup(self):
    ...
    self.assertEqual(o2_again.status, 'SENT')
    # now the sent order o3 should be the same
    o3_again = models.Order.objects.get(pk=o3.pk)
    self.assertEqual(o3_again.status, 'SENT')

  def test_unicode_errors(self):
    """Makes sure that funny characters don't mess up the template system"""
    c = Client()
    response = c.get(
      '/lemur/order/build/?whichForm=search&csrfmiddlewaretoken=3ff87b8c8ea9d6ae6f2797eaa30509f4&title=harry%20potter&page=3&author=rowling')
  ...
  def test_creation_date(self):
    ...
    order.save()
    self.assertEqual(order.date_opened, date)
